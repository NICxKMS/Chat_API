...
export function classifyModels(loadedModelList) {
  const models = Array.isArray(loadedModelList?.models) ? loadedModelList.models : [];
...
export function classifyModelsWithCriteria(loadedModelList, criteria = {}) {
  const models = Array.isArray(loadedModelList?.models) ? loadedModelList.models : [];
  const enhanced = enhanceModelsWithClassifier(models);
  const availableProps = availableClassificationProperties();
  const useHier = criteria && criteria.hierarchical === false ? false : true;
  if (useHier) {
    const hierarchicalGroups = buildModelHierarchy(enhanced);
    return {
      classified_groups: [],
      available_properties: availableProps,
      error_message: "",
  ...
  // Flat grouping by requested properties
  const props = Array.isArray(criteria.properties) && criteria.properties.length
    ? criteria.properties
    : ["provider", "family", "type", "capability"];
  const groups = [];
  const pushGroup = (name, value, modelsList) => {
    groups.push({ property_name: name, property_value: value, models: modelsList });
  };
  for (const property of props) {
    ...
    for (const m of enhanced) {
      ...
      let val = "";
      if (property === "provider") val = m.provider || "";
      else if (property === "family") val = m.family || "";
      else if (property === "type") val = m.model_type || m.type || "";
      else if (property === "series") val = m.series || "";
      else if (property === "variant") val = m.variant || "";
      else if (property === "capability") {
        for (const c of (m.capabilities || [])) {
          if (!map.has(c)) map.set(c, []);
          map.get(c).push(model);
        }
        continue;
      } else if (property === "context_window") {
        val = categorizeContextWindow(m.context_size | 0);
      } else if (property === "multimodal") {
        val = (m.is_multimodal ? "Yes" : "No");
      } else {
        continue;
      }
      if (!map.has(val)) map.set(val, []);
      map.get(val).push(model);
    }
    const entries = Array.from(map.entries());
    if (property === "capability") entries.sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
    for (const [value, list] of entries) pushGroup(property, value, list);
  }
  return { classified_groups: groups, available_properties: availableProps, error_message: "", hierarchical_groups: [] };
}
  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname === "/classify") {
      try {
        ...
        const response = classifyModels({ models: Array.isArray(body?.models) ? body.models : [] });
